...
double _Complex wlm_PL(int const l, int const m, int const d1, int const d2, int const d3, double const m1, double const m2, double const q2, int const N_L, int const size)
{
	gsl_set_error_handler_off();
	double mp1= m1 * N_L / (2.0 * M_PI);
	double mp2= m2 * N_L / (2.0 * M_PI);
	...
	double _Complex gen_zeta=t1+t2+t3;
	 ...
	 /*get w_lm*/
 	 double twopiL = 2.0*M_PI/((double) N_L);	 

	 double dbl_l = (double) l;
	 double dsq = pow(dvec[0],2)+pow(dvec[1],2)+pow(dvec[2],2);

	 double Est = (2.0*M_PI/N_L)*( sqrt(pow(mp1,2) + q2) + sqrt(pow(mp2,2) + q2) );
	 double E = sqrt(pow(Est,2) + pow(twopiL,2)*dsq);
	 double gam_val = lorentz_gamma_scalar(E, dvec,N_L);
	 double complex q = csqrt(q2);
	 double complex denom = pow(M_PI,3./2.)*sqrt(2*dbl_l+1.0)*gam_val*cpow(q,dbl_l+1.0);
	 double complex wlm = gen_zeta/denom;				/* eq 96 Rummukainen Gottlieb*/
